  ...
  describe Value do
    context "serialization" do
      it "can write and read to io" do
        value = Value[0_i64, 100, 1]
        io = IO::Memory.new
        value.to_io(io)
        io.rewind
        Value.from_io(io).should eq value

        value = Value[0_i64, 100, 1.1]
        io = IO::Memory.new
        value.to_io(io)
        io.rewind
        Value.from_io(io).should eq value

        value = Value[0_i64, 100, true]
        io = IO::Memory.new
        value.to_io(io)
        io.rewind
        Value.from_io(io).should eq value

        value = Value[0_i64, 100, "string"]
        io = IO::Memory.new
        value.to_io(io)
        io.rewind
        Value.from_io(io).should eq value

        value = Value[0_i64, 100, ["array"]]
        io = IO::Memory.new
        value.to_io(io)
        io.rewind
        Value.from_io(io).should eq value
    ...
    context "Value[]" do
      ...
      it "creates an Array value" do
        value = Value[0_i64, 100, [1, "string", true, 1.1]]
